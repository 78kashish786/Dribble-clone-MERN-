...
const UpdatePost = () => {
    ...
    const getSinglePost = async()=>{
        try{
            ...
            // const{photoData} = await axios.get('http://localhost:8080/api/v1/')
            setTitle(data?.singlePost?.title)
            // setPhoto(data?.singlePost?.photo)
            setDesc(data?.singlePost?.description)
            setSingleCategory(data?.singlePost?.category?.name);
            setCatId(data?.singlePost._id)
